...
def get_data(
	...
	# print("168 = Accounts>>>",root_type,custom_sub_report_type,accounts)
...
def add_total_row(out, root_type,custom_sub_report_type, balance_must_be, period_list, company_currency):
	total_row = {
		"account_name": _("Total {0}-[{1}]({2})").format(_(root_type), _(custom_sub_report_type), _(balance_must_be)),
		"account": _("Total {0}-[{1}]({2})").format(_(root_type), _(custom_sub_report_type), _(balance_must_be)),
		"currency": company_currency,
		"opening_balance": 0.0,
	}

	for row in out:
		if row.get("custom_sub_report_type") != custom_sub_report_type:
...
def get_accounts(company, root_type,custom_sub_report_type):
	# return frappe.db.sql(
	# 	"""
	# 	select name, account_number, parent_account, lft, rgt, root_type, report_type, custom_sub_report_type, account_name, include_in_gross, account_type, is_group, lft, rgt
	# 	from `tabAccount`
	# 	where company=%s and root_type=%s and custom_sub_report_type=%s order by lft""",
	# 	(company, root_type,custom_sub_report_type),
	# 	as_dict=True,
	# )
	return frappe.db.sql(
		"""
...
def set_gl_entries_by_account(
	...
	if custom_sub_report_type:
		account_filters.update(
			{
				"custom_sub_report_type": custom_sub_report_type,
			}
		)
