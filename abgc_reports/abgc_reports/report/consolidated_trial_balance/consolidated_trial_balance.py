...
def get_data(filters, companies=None):

	if filters.consolidated:
		company_filter = f"""company IN ({','.join(f'"{comp}"' for comp in companies)})"""
		con_group = "group by company"
	else:
		company_filter = 'company ="{0}"'.format(filters.company)
		con_group = ""

	accounts = frappe.db.sql(
		f"""select name, account_number, parent_account, account_name, root_type, report_type, lft, rgt

		from `tabAccount` where {company_filter} order by lft""",
		as_dict=True,
	)
	...
	if filters.consolidated:
		...
		for company in companies:
			...
			total_row.update(
				{
					company + "-closing_debit": 0,
					company + "-closing_credit": 0,
				}
			)
			...
			process_raw_data(data, final_consolidated_data, filters.company, total_row)

		final_consolidated_data.extend([{}, total_row])
...
def process_raw_data(data, final_consolidated_data, company, total_row):
	x = [i.get("account_name") for i in final_consolidated_data]
	for entry in data:
		idx = check_account(entry.get("account_name"), final_consolidated_data)
		if idx:
			final_consolidated_data[idx].update(
				{
					company + "-closing_debit": entry.get("closing_debit"),
					company + "-closing_credit": entry.get("closing_credit"),
				}
			)
		else:
			closing_debit = entry.pop("closing_debit")
			closing_credit = entry.pop("closing_credit")
			entry.update(
				{
					company + "-closing_debit": closing_debit,
					company + "-closing_credit": closing_credit,
				}
			)
			if entry.get("account_name") not in x:
				final_consolidated_data.append(entry)

			else:
				for i in final_consolidated_data:
					if entry.get("account_name") == i.get("account_name"):
						i[company + "-closing_debit"] = closing_debit
						i[company + "-closing_credit"] = closing_credit

	calculate_total(final_consolidated_data, total_row, company)
